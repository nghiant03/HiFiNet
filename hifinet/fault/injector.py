...
class FaultInjector:
    ...
    def inject(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.exclude:
            clean = (
                data[data["node_id"].isin(self.config.exclude)]
                .copy()
        ...
        match self.config.mode:
            case "singular":
                for idx, (fault_name, type_idx) in enumerate(
                    self.config.type_mapping.items()
                ):
                    node_data = (
                        data[data["node_id"] == node_ids[idx]]
                        .copy()
                        .reset_index(drop=True)
                    )
                    ...
                    fault = fault_class(
                        **self.config.fault_config[fault_name].model_dump()
                    )
